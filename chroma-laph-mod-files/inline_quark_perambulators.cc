                ...
                GaugeConfigurationInfo &gaugeinfo)
{
 ...
 xmlread(xmlrd,"SourceTime",source_times,"QUARK_PERAMBULATORS");
 for (int k=0;k<source_times.size();++k){
    comp.SourceTimes.insert(source_times[k]);}
 ...
 QDPIO::cout <<endl<<"Source Times: ";
 for (set<int>::const_iterator st=comp.SourceTimes.begin();
                st!=comp.SourceTimes.end();++st){
    QDPIO::cout <<" "<<*st;}
                                              ...
                                              XMLWriter& xmlout) 
{
 ...
 for (set<int>::const_iterator st=comp.SourceTimes.begin();
                st!=comp.SourceTimes.end();++st){
    Q.computePerambulators(*st,comp.SourceLaphEigvecIndices,
 ...
 END_CODE();
    ...
    for (set<int>::const_iterator st=comp.SourceTimes.begin();
                st!=comp.SourceTimes.end();++st){
    QI.computePerambulatorsInsertion(*st,comp.SourceLaphEigvecIndices,
                        comp.SinkTimes,verbose);}
