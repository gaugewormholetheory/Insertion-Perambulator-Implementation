...
cmplx zetaRGL_der(int l, int m, const std::vector<double>& s,
              double gam, double usq, const std::vector<double>& s_der, double gam_der, double usq_der, const std::vector<double>& th1_der );
...
class YLMpoly_der
{
   ...
   int m_lval_shift, m_mval_shift;
   double m_cf_shift;
   bool m_x3flag_shift;
   int m_pdeg_shift;
   std::vector<double> m_pcf_shift;
   mutable std::vector<double> m_buf_shift;

   int m_lval_even, m_mval_even;
   double m_cf_even;
   bool m_x3flag_even;
   int m_pdeg_even;
   std::vector<double> m_pcf_even;
   mutable std::vector<double> m_buf_even;

   int m_lval_odd, m_mval_odd;
   double m_cf_odd;
   bool m_x3flag_odd;
   int m_pdeg_odd;
   std::vector<double> m_pcf_odd;
   mutable std::vector<double> m_buf_odd;


 public:

   YLMpoly_der(int lvalue, int mvalue);
   YLMpoly_der(const YLMpoly& in);
   YLMpoly_der& operator=(const YLMpoly_der& in);
   ~YLMpoly_der(){}

   cmplx value(double x1, double x2, double x3, double x1_der, double x2_der, double x3_der) const;
   double realpart(double x1, double x2, double x3) const;
   double imagpart(double x1, double x2, double x3) const;
 ...
 private:
   ...
   double polarpoly(double zz, double rr) const;
   double polarpoly_shift(double zz, double rr) const;
   double polarpoly_even(double zz, double rr) const;
   double polarpoly_odd(double zz, double rr) const;
   cmplx azimuthal_m1(double x1, double x2) const;
   cmplx azimuthal_shift(double x1, double x2) const;
   cmplx azimuthal_even(double x1, double x2) const;
   cmplx azimuthal_odd(double x1, double x2) const;
   double azimuthal_realpart(double x1, double x2) const;
   double azimuthal_imagpart(double x1, double x2) const;
