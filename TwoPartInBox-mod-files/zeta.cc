...
YLMpoly::YLMpoly(int lvalue, int mvalue)
{
 ...
 m_lval=lvalue;
 m_mval=mvalue;
 m_pdeg=(lvalue-mvalue)/2; // upper sum limit with floor
 m_pcf.resize(m_pdeg+1); 
 m_buf.resize(m_pdeg+1);
 m_x3flag=(m_lval-m_mval)>(2*m_pdeg); // 3-2

 if (m_lval==1){
  if (m_mval==0){
     m_cf=  0.48860251190291992159;  // irrational common factor
     m_pcf[0]=1.0;} // k_dependent factor, polynomial is organized as follows: (r^2)^k z
  else if (m_mval==1){
...
// polar polynomial  * azimuthal part * possible extra z
...
double YLMpoly::polarpoly(double zz, double rr) const
{
 ...
 for (int k=m_pdeg-1;k>=0;k--)
    m_buf[k]=m_buf[k+1]*rr; // m_buf[k]=r**(m_pdeg-k)
 double res=m_buf[0]*m_pcf[0];
 double zzpow=1.0;
 for (int k=1;k<=m_pdeg;k++){
    zzpow*=zz; // zzpow(k) = zz^k
...
// without the m_cf values A_m and B_m
...
YLMpoly_der::YLMpoly_der(int lvalue, int mvalue)
{
 if ((lvalue<0)||(lvalue>2)){
    throw(std::invalid_argument("Unsupported value of L in YLMpoly_der"));}
 if ((mvalue<0)||(mvalue>lvalue)){
    throw(std::invalid_argument("Unsupported or invalid value of M in YLMpoly_der"));}
 m_lval=lvalue;
 m_mval=mvalue;
 ...
 m_lval_shift=lvalue-1;
 m_mval_shift=mvalue+1;
 m_pdeg_shift=( (lvalue-1)-(mvalue+1) )/2;
 if (m_pdeg_shift >= 0) {
    m_pcf_shift.resize(m_pdeg_shift+1);
    m_buf_shift.resize(m_pdeg_shift+1);
} else {
    m_pdeg_shift = -1;
}
 //m_pcf_shift.resize(m_pdeg_shift+1);
 //m_buf_shift.resize(m_pdeg_shift+1);
 m_x3flag_shift=(m_lval_shift-m_mval_shift)>(2*m_pdeg_shift);
 //std::cout<< m_lval_shift << " " << m_mval_shift << std::endl;
 //std::cout<< m_pdeg_shift << "  " << m_pcf_shift.size() << " " << m_buf_shift.size() << " " << m_x3flag_shift << std::endl;
 
 m_lval_even=lvalue;
 m_mval_even=mvalue+2;
 m_pdeg_even=( (lvalue)-(mvalue+2) )/2;
 if (m_pdeg_even >= 0) {
    m_pcf_even.resize(m_pdeg_even+1);
    m_buf_even.resize(m_pdeg_even+1);
} else {
    m_pdeg_even = -1;
}
 m_x3flag_even=(m_lval_even-m_mval_even)>(2*m_pdeg_even);
 //std::cout<< m_lval_even << " " << m_mval_even << std::endl;
 //std::cout<< m_pdeg_even << "  " << m_pcf_even.size() << " " << m_buf_even.size() << " " << m_x3flag_even << std::endl;

 m_lval_odd=lvalue;
 m_mval_odd=mvalue;
 m_pdeg_odd=(lvalue-mvalue)/2;
 if (m_pdeg_odd >= 0) {
    m_pcf_odd.resize(m_pdeg_odd+1);
    m_buf_odd.resize(m_pdeg_odd+1);
} else {
    m_pdeg_odd = -1;
}
 m_x3flag_odd=(m_lval_odd-m_mval_odd)>(2*m_pdeg_odd);
 //std::cout<< m_lval_odd << " " << m_mval_odd << std::endl;
 //std::cout<< m_pdeg_odd << "  " << m_pcf_odd.size() << " " << m_buf_odd.size() << " " << m_x3flag_odd << std::endl;
 ...
 if (m_lval==1){
  if (m_mval==0){
     ...
     m_cf_even=0.0;
     m_pcf_even[0]=0.0;
     m_cf_odd=  0.48860251190291992159;
     m_pcf_odd[0]=1.0;
     } 
  else if (m_mval==1){
     m_cf= -0.34549414947133547927;
     m_pcf[0]=1.0;
     m_cf_shift=0.0;
     //m_cf_even=-0.34549414947133547927;
     m_cf_even=0.0;
     m_cf_odd= 0.0;
     m_pcf_odd[0]=0.0;}
  }
  ...
  if (m_mval==0){
     ...
     m_pcf[0]=-1.0;  
     m_pcf[1]=3.0;  
     m_cf_shift= 0.48860251190291992159;
     m_pcf_shift[0]=1.0;
     m_cf_even= 1.8923494076551208;
     m_pcf_even[0]= 1.0;
     m_cf_odd= 0.0;
     m_pcf_odd[0]= 0.0;
     m_pcf_odd[1]= 0.0;}
...
// polar polynomial  * azimuthal part * possible extra z
cmplx YLMpoly_der::value(double x1, double x2, double x3, double x1_der, double x2_der, double x3_der) const
{
 if (m_lval==0) return cmplx(0.0,0.0);
 double rr=x1*x1+x2*x2+x3*x3;
 double zz=x3*x3;
 double x_der = x1_der;
 double y_der = x2_der;
 ...
 res1=azimuthal_shift(x1,x2);
 if (m_pdeg_shift>0) res1*=polarpoly_shift(zz,rr);
 if (m_x3flag_shift) res1*=x3;
 res1*= (x1*x_der+x2*y_der+x3*z_der);
 //std::cout<< res1 << std::endl;

 if ( (m_lval-m_mval)%2==0 ){
 	res2=azimuthal_even(x1,x2);
 	if (m_pdeg_even>0) res2*=polarpoly_even(zz,rr);
 	if (m_x3flag_even) res2*=x3;
 	res2*= (x3*z_der);
 }
 else if ( (m_lval-m_mval)%2==1 ){
 	res2=azimuthal_odd(x1,x2);
        if (m_pdeg_odd>0) res2*=polarpoly_odd(zz,rr);
        if (m_x3flag_odd) res2*=x3;
        if (x3!=0){
		res2*= (z_der/x3);
	}
 }
 ...
 res3=azimuthal_m1(x1,x2);
 if (m_pdeg>0) res3*=polarpoly(zz,rr);
 if (m_x3flag) res3*=x3;
 res3*= cmplx(m_mval,0.0) * cmplx(x_der,y_der);
...
double YLMpoly_der::realpart(double x1, double x2, double x3) const
{
 if (m_lval==0) return 0.28209479177387814348;
 double rr=x1*x1+x2*x2+x3*x3;
 double zz=x3*x3;
...
double YLMpoly_der::imagpart(double x1, double x2, double x3) const
{
 if (m_lval==0) return 0.0;
 double rr=x1*x1+x2*x2+x3*x3;
 double zz=x3*x3;
...
double YLMpoly_der::polarpoly(double zz, double rr) const
{
 m_buf[m_pdeg]=1.0;
 for (int k=m_pdeg-1;k>=0;k--)
    m_buf[k]=m_buf[k+1]*rr; // m_buf[k]=r**(m_pdeg-k)
...
double YLMpoly_der::polarpoly_shift(double zz, double rr) const
{
 
 if (m_lval_shift < m_mval_shift) return 0.0;
 
 m_buf_shift[m_pdeg_shift]=1.0;
 for (int k=m_pdeg_shift-1;k>=0;k--)
    m_buf_shift[k]=m_buf_shift[k+1]*rr; // m_buf[k]=r**(m_pdeg-k)
 double res=m_buf_shift[0]*m_pcf_shift[0];
 double zzpow=1.0;
 for (int k=1;k<=m_pdeg_shift;k++){
    zzpow*=zz; // zzpow(k) = zz^k
    res+=m_buf_shift[k]*zzpow*m_pcf_shift[k];}
 return res;
 
}

double YLMpoly_der::polarpoly_even(double zz, double rr) const
{
 m_buf_even[m_pdeg_even]=1.0;
 for (int k=m_pdeg_even-1;k>=0;k--)
    m_buf_even[k]=m_buf_even[k+1]*rr; // m_buf[k]=r**(m_pdeg-k)
 double res=m_buf_even[0]*m_pcf_even[0];
 double zzpow=1.0;
 for (int k=1;k<=m_pdeg_even;k++){
    zzpow*=zz; // zzpow(k) = zz^k
    res+=m_buf_even[k]*zzpow*m_pcf_even[k];}
 return res;
}


double YLMpoly_der::polarpoly_odd(double zz, double rr) const
{
 m_buf_odd[m_pdeg_odd]=1.0;
 for (int k=m_pdeg_odd-1;k>=0;k--)
    m_buf_odd[k]=m_buf_odd[k+1]*rr; // m_buf[k]=r**(m_pdeg-k)
 double res=m_buf_odd[0]*m_pcf_odd[0];
 double zzpow=1.0;
 for (int k=1;k<=m_pdeg_odd;k++){
    zzpow*=zz; // zzpow(k) = zz^k
    res+=m_buf_odd[k]*zzpow*m_pcf_odd[k];}
 return res;
}

   //  This evaluates  m_cf * (x1 + I * x2) ^ m_mvalue

cmplx YLMpoly_der::azimuthal_shift(double x1, double x2) const
{
 if (m_mval==0) return cmplx(m_cf_shift,0.0);
 else if (m_mval==1) return cmplx(m_cf_shift*x1,m_cf_shift*x2);
 cmplx xy(x1,x2),res(1.0,0.0);
 int powkey=m_mval;
 while (powkey>0){
    if (powkey & 1){
       res*=xy;}
    xy*=xy; powkey>>=1;}
 return m_cf_shift*res;
}

cmplx YLMpoly_der::azimuthal_m1(double x1, double x2) const
{
 if (m_mval==0) return cmplx(0.0,0.0);
 if (m_mval-1==0) return cmplx(m_cf,0.0);
 else if (m_mval-1==1) return cmplx(m_cf*x1,m_cf*x2);
 cmplx xy(x1,x2),res(1.0,0.0);
 int powkey=m_mval-1;
 while (powkey>0){
    if (powkey & 1){
...
cmplx YLMpoly_der::azimuthal_even(double x1, double x2) const
{
 if (m_mval==0) return cmplx(m_cf_even,0.0);
 else if (m_mval==1) return cmplx(m_cf_even*x1,m_cf_even*x2);
 cmplx xy(x1,x2),res(1.0,0.0);
 int powkey=m_mval;
 while (powkey>0){
    if (powkey & 1){
       res*=xy;}
    xy*=xy; powkey>>=1;}
 return m_cf_even*res;
}

cmplx YLMpoly_der::azimuthal_odd(double x1, double x2) const
{
 if (m_mval==0) return cmplx(m_cf_odd,0.0);
 else if (m_mval==1) return cmplx(m_cf_odd*x1,m_cf_odd*x2);
 cmplx xy(x1,x2),res(1.0,0.0);
 int powkey=m_mval;
 while (powkey>0){
    if (powkey & 1){
...
double YLMpoly_der::azimuthal_realpart(double x1, double x2) const
{
 switch (m_mval){
 case 0:
    return m_cf;
...
double YLMpoly_der::azimuthal_imagpart(double x1, double x2) const
{
 switch (m_mval){
 case 0:
    return 0.0;
...
double lzero_func_der(double usq)
{
 if (usq>=0.0){
    double u=sqrt(usq);
    return exp(usq)*dawson(u)/u;}
 else{
    double u=sqrt(-usq);
    const double pisqrt=1.772453850905516027298;
    return pisqrt*erf(u)/(2.0*u);}
}
...
class ZetaSummand1s_der
{
    double gam,s1,s2,s3,usq,ss,s1_der,s2_der,s3_der,gam_der,usq_der,t1,t2,t3;
    YLMpoly ylm;
    YLMpoly_der ylm_der;
 public:

    ZetaSummand1s_der(int lval, int mval, const vector<double>& s, double ss,
                  double gam, double usq, const vector<double>& s_der, double gam_der, double uu_der, const vector<double>& th1_der );
    cmplx operator()(int n1, int n2, int n3) const;
};


ZetaSummand1s_der::ZetaSummand1s_der(int lval, int mval, const vector<double>& s,
                             double ss_, double gam_, double uu_, const vector<double>& s_der, double gam_der_, double uu_der_, const vector<double>& th1_der) : ylm(lval,mval),ylm_der(lval,mval)
{
 s1=s[0];
 s2=s[1];
 s3=s[2];
...
inline cmplx ZetaSummand1s_der::operator()(int n1, int n2, int n3) const
{
 //std::cout<< "s1= "<<s1<<"  "<<"s2= "<<s2<<"  "<<"s3= "<<s3<<"  "<<"ss= "<<ss<<std::endl;
 
 const double pi=3.14159265358979323846264;
 double z1=n1-(s1/gam)*(0.5+(gam-1)*(n1*s1+n2*s2+n3*s3)/ss);
 double z2=n2-(s2/gam)*(0.5+(gam-1)*(n1*s1+n2*s2+n3*s3)/ss);
 double z3=n3-(s3/gam)*(0.5+(gam-1)*(n1*s1+n2*s2+n3*s3)/ss);
 double zz=z1*z1+z2*z2+z3*z3;
 
  //std::cout<< "z1= "<<z1<<"  "<<"z2= "<<z2<<"  "<<"z3= "<<z3<<"  "<<"zz= "<<zz<<std::endl;

 double z1_der=(t1/(2*pi))+(s1*gam_der/(gam*gam)-s1_der/gam)*(0.5+(gam-1)*(n1*s1+n2*s2+n3*s3)/ss) - (s1/gam)*( gam_der*(n1*s1+n2*s2+n3*s3)/ss + (gam-1)*( (s1*t1+s2*t2+s3*t3)/(2*pi*ss) + (n1*s1_der+n2*s2_der+n3*s3_der)/ss 
			 - 2.0 * (n1*s1+n2*s2+n3*s3) * (s1*s1_der+s2*s2_der+s3*s3_der)/(ss*ss)    ) );
 double z2_der=(t2/(2*pi))+(s2*gam_der/(gam*gam)-s2_der/gam)*(0.5+(gam-1)*(n1*s1+n2*s2+n3*s3)/ss) - (s2/gam)*( gam_der*(n1*s1+n2*s2+n3*s3)/ss + (gam-1)*( (s1*t1+s2*t2+s3*t3)/(2*pi*ss) + (n1*s1_der+n2*s2_der+n3*s3_der)/ss 
                         - 2.0 * (n1*s1+n2*s2+n3*s3) * (s1*s1_der+s2*s2_der+s3*s3_der)/(ss*ss)    ) );
 double z3_der=(t3/(2*pi))+(s3*gam_der/(gam*gam)-s3_der/gam)*(0.5+(gam-1)*(n1*s1+n2*s2+n3*s3)/ss) - (s3/gam)*( gam_der*(n1*s1+n2*s2+n3*s3)/ss + (gam-1)*( (s1*t1+s2*t2+s3*t3)/(2*pi*ss) + (n1*s1_der+n2*s2_der+n3*s3_der)/ss 
                         - 2.0 * (n1*s1+n2*s2+n3*s3) * (s1*s1_der+s2*s2_der+s3*s3_der)/(ss*ss)    ) );

 double zz_der = 2.0*(z1*z1_der+z2*z2_der+z3*z3_der);  
 //std::cout<< "z1_der= "<<z1_der<<"  "<<"z2_der= "<<z2_der<<"  "<<"z3_der= "<<z3_der<<"  "<<"zz_der= "<<zz_der<<std::endl;
 return 1.0/(zz-usq)*exp(-(zz-usq))*( ylm_der.value(z1,z2,z3,z1_der,z2_der,z3_der) - ylm.value(z1,z2,z3) * (1.0 + 1.0/(zz-usq) ) * (zz_der - usq_der) );
}
...
inline const GQvec<cmplx>& ZetaSummand2s::operator()(int n1, int n2, int n3) const
{
 ...
 cmplx phase(cos(pi*wb),sin(pi*wb)); // 1st exp
 wb*=alpha;
 double w1=n1+wb*s1;
 double w2=n2+wb*s2;
 double w3=n3+wb*s3;
 double ww=w1*w1+w2*w2+w3*w3;
 phase*=ylm.value(w1,w2,w3); // 1st times 2nd exp
 for (int k=0;k<tvals.npoints();k++)
    summands[k]=phase*exp(-pipi*ww/tvals[k]); // 1st times 2nd times t-dep phase
 return summands;
}
...
class ZetaSummand2s_der
{
 ...
 public:
    ...
    ZetaSummand2s_der(int lval, int mval, const vector<double>& s, double ss,
                  double gam, double usq, const GQvec<double>& tvalues, const vector<double>& s_der, double gam_der, double uu_der, const vector<double>& th1_der );
    const GQvec<cmplx>& operator()(int n1, int n2, int n3) const;
};                          

 
ZetaSummand2s_der::ZetaSummand2s_der(int lval, int mval, const vector<double>& s,
                            double ss_, double gam_, double uu_,
                            const GQvec<double>& tvalues, const vector<double>& s_der, double gam_der_, double uu_der_, const vector<double>& th1_der )
                           : ylm(lval,mval), ylm_der(lval,mval), tvals(tvalues)
{
 s1=s[0];
 s2=s[1];
 s3=s[2];
...
inline const GQvec<cmplx>& ZetaSummand2s_der::operator()(int n1, int n2, int n3) const
{
 const double pi=3.14159265358979323846264;
 const double pipi=9.86960440108935861883447;
 double wb=n1*s1+n2*s2+n3*s3;
 ...
 double w1_der=(n1*s1+n2*s2+n3*s3/ss)*gam_der*s1+(gam-1)*( s1 * (n1*s1_der+n2*s2_der+n3*s3_der)/ss - 2.0 * (n1*s1+n2*s2+n3*s3)/(ss*ss) * (s1*s1_der+s2*s2_der+s3*s3_der) + (n1*s1+n2*s2+n3*s3)/(ss*ss) * s1_der );
 double w2_der=(n1*s1+n2*s2+n3*s3/ss)*gam_der*s2+(gam-1)*( s2 * (n1*s1_der+n2*s2_der+n3*s3_der)/ss - 2.0 * (n1*s1+n2*s2+n3*s3)/(ss*ss) * (s1*s1_der+s2*s2_der+s3*s3_der) + (n1*s1+n2*s2+n3*s3)/(ss*ss) * s2_der  );
 double w3_der=(n1*s1+n2*s2+n3*s3/ss)*gam_der*s3+(gam-1)*( s3 * (n1*s1_der+n2*s2_der+n3*s3_der)/ss - 2.0 * (n1*s1+n2*s2+n3*s3)/(ss*ss) * (s1*s1_der+s2*s2_der+s3*s3_der) + (n1*s1+n2*s2+n3*s3)/(ss*ss) * s3_der  );
 
 for (int k=0;k<tvals.npoints();k++)
    summands[k]=phase*exp(-pipi*ww/tvals[k])*( ylm.value(w1,w2,w3) * (cmplx(0.0,pi)*(n1*s1_der+n2*s2_der+n3*s3_der) - (pipi/tvals[k])*(w1*w1_der+w2*w2_der+w3*w3_der) - cmplx(0.0,1.0) * (n1*t1+w2*t2+w3*t3) ) + ylm_der.value(w1,w2,w3) ) ;
 return summands;
}
...
cmplx zeta_noshift_der(int l, int m, double usq)
{
 if (zetaRGL_RestFrame_IsZero(l,m)) return cmplx(0.0,0.0);
 const double pi=3.14159265358979323846264;
...
/*
 ...
 cmplx res2(0.0,0.0);
 if (l==0){
    res2+=pi*lzero_func(usq)*gam_der;
    res2+=gam*pi*lzero_func_der(usq)*usq_der;}
*/
 return res1+res2;
...
cmplx zeta_shifted_der(int l, int m, const vector<double>& s, double ss,
                   double gam, double usq, const vector<double>& s_der, double gam_der,
                   double usq_der, const vector<double>& th1_der )
{
 std::cout<<"gam="<<gam<<"  "<<"ss="<<ss<<"  "<<"usq="<<usq<<"  "<<std::endl; 
 const double pi=3.14159265358979323846264;
 // Term 1
 cmplx res1;
 ZetaSummand1s_der summand1(l,m,s,ss,gam,usq,s_der,gam_der,usq_der,th1_der);
 int nnmin=ceil(std::abs(s[0]));
 int nn=ceil(std::abs(s[1])); if (nn>nnmin) nnmin=nn;
 nn=ceil(std::abs(s[2])); if (nn>nnmin) nnmin=nn;
 nn=ceil(sqrt(std::abs(usq))); if (nn>nnmin) nnmin=nn;
 nnmin+=6;
 double abstol=1e-18, reltol=1e-12;
 do_sum<ZetaSummand1s_der,cmplx>(res1,summand1,nnmin,abstol,reltol);
 std::cout<< " The first term is equal to " << res1<< std::endl;  
 ...
 cmplx res31(0.0,0.0);
 for (int k=0;k<tvals.npoints();k++){  
    res31+=wts[k]*intgrand[k]*exp(tvals[k]*usq)/pow(tvals[k],1.5+double(l));}
 cmplx I(0.0,1.0),Il(1.0,0.0);
 for (int k=1;k<=l;k++) Il*=I;
 res31*=Il*gam*pow(pi,1.5+double(l));
 res31*= gam_der/gam;
 std::cout<< " The third term part 1 is equal to " << res31<< std::endl;
 ...
 GQvec<cmplx> intgrand;
 nnmin=6;
 do_sum<ZetaSummand2s,GQvec<cmplx> >(intgrand,summand2,nnmin,abstol,reltol,true);*/ // that part is the same as in the previous sum only the t degree outside the sum differs
 cmplx res32(0.0,0.0);
 for (int k=0;k<tvals.npoints();k++){
    res32+=wts[k]*intgrand[k]*exp(tvals[k]*usq)/pow(tvals[k],0.5+double(l));} //power of t was changed
 /*cmplx I(0.0,1.0),Il(1.0,0.0);
 for (int k=1;k<=l;k++) Il*=I;*/
 res32*=Il*gam*pow(pi,1.5+double(l))*usq_der;
 std::cout<< " The third term part 2 is equal to " << res32<< std::endl;
 std::cout<<"gam="<<gam<<"  "<<"ss="<<ss<<"  "<<"usq="<<usq<<"  "<<std::endl;
 // Term 3.3
 /*GQvec<double> tvals,wts;
 assign_gq_abscissae_weights(tvals,wts);*/
 ZetaSummand2s_der summand2_der(l,m,s,ss,gam,usq,tvals,s_der,gam_der,usq_der,th1_der);
 GQvec<cmplx> intgrand33;
 //nnmin=6;
 do_sum<ZetaSummand2s_der,GQvec<cmplx> >(intgrand33,summand2_der,nnmin,abstol,reltol,true);
 cmplx res33(0.0,0.0);
 for (int k=0;k<tvals.npoints();k++){
    res33+=wts[k]*intgrand33[k]*exp(tvals[k]*usq)/pow(tvals[k],1.5+double(l));}
 /*I = (0.0,1.0); Il = (1.0,0.0);
 for (int k=1;k<=l;k++) Il*=I;*/
 res33*=Il*gam*pow(pi,1.5+double(l));
 std::cout<< " The third term part 3 is equal to " << res33<< std::endl;
 ...
 if (l==0){
    res2+=pi*lzero_func(usq)*gam_der;
    res2+=gam*pi*lzero_func_der(usq)*usq_der;}
 std::cout<< " The second term is equal to " << res2<< std::endl;   

 return res1+res2+res31+res32+res33;             
}
...
cmplx zetaRGL_der(int l, int m, const vector<double>& s, double gam, double usq, const vector<double>& s_der, double gam_der, double usq_der, const vector<double>& th1_der)
{
 double ss=s[0]*s[0]+s[1]*s[1]+s[2]*s[2];
 if (ss<1e-15){
    std::cout<< "The derivative was not yet implemented for the faster noshift_case" << std::endl;
    /*return zeta_noshift_der(l,m,usq);*/}
 return zeta_shifted_der(l,m,s,ss,gam,usq,s_der,gam_der,usq_der,th1_der);
}
